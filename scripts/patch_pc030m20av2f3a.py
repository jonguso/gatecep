...
svc=root/"mobile/src/services/brokers/brokerFeeScheduleManagementService.js"
for required in (p,svc):
    if not required.exists(): raise SystemExit(f"ERROR — required file missing: {required}")
...
# fee status on connected account
...
# robust styles insertion after StyleSheet.create({
anchor="const styles = StyleSheet.create({"
...
if "feePanel:" not in src:
    if anchor not in src: raise SystemExit("ERROR — StyleSheet.create anchor missing.")
    src=src.replace(anchor,styles,1)

if src!=orig:
    p.with_suffix(p.suffix+".pc030m20av2f3a.bak").write_text(orig,encoding="utf-8")
    p.write_text(src,encoding="utf-8")
    print("UPDATED — Broker Accounts fee management applied.")
    print("UPDATED — style insertion now uses stable StyleSheet.create anchor.")
else:
    print("NO CHANGE — AV2F3A already applied.")