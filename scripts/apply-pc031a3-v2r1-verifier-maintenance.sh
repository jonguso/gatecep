...
if "A4 broker submission is centralized behind mode-aware routing" in s:
    print("Verifier already updated; no change needed.")
    raise SystemExit(0)

start_marker = "if grep -n 'placeBrokerOrder'"
end_marker = "\ngrep -q 'id: \"GATECEP_PRACTICE\"' mobile/src/services/brokers/brokerRegistry.js"
start = s.find(start_marker)
if start < 0:
    raise SystemExit("Could not find stale placeBrokerOrder verifier block start")
end = s.find(end_marker, start)
if end < 0:
    raise SystemExit("Could not find stale placeBrokerOrder verifier block end")

replacement = """if grep -n 'placeBrokerOrder' \\
...
print("PC-031A3-V2-R1 maintenance applied.")
PY

echo
echo "=== UPDATED VERIFIER DIFF ==="
