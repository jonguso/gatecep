...
import { buildPortfolioAwareInvestorAlert } from "../../src/features/intelligence/portfolioAwareInvestorAlertService";
...
export default function Calendar() {
  ...
  const load = useCallback(async () => {
    ...
    const [localResult, remoteResult, portfolioResult, targetResult] = await Promise.allSettled([
...
function EventDetail({ event, holdings, sectorTargets }) { const alert = buildPortfolioAwareInvestorAlert({ evidence: event, holdings, sectorTargets }); return <View style={s.row}><View style={s.eventMain}><View style={s.badges}><View style={[s.badge, event.trustLevel === "OFFICIAL" ? s.official : s.verified]}><Text style={s.badgeText}>{event.trustLevel === "OFFICIAL" ? "Official" : event.trustLevel === "REPORTED" ? "Reported" : "Verified"}</Text></View><Text style={s.type}>{event.type.replaceAll("_", " ")}</Text></View><Text style={s.eventTitle}>{event.title}</Text><Text style={s.company}>{event.company}</Text><Text style={s.body}>{event.detail}</Text><Text style={s.source}>{event.source}</Text><Pressable onPress={() => router.push({ pathname: "/investor-alert-review", params: { alert: JSON.stringify(alert) } })}><Text style={s.open}>Review portfolio impact ›</Text></Pressable>{event.url ? <Pressable onPress={() => Linking.openURL(event.url)}><Text style={s.open}>Open original evidence ↗</Text></Pressable> : null}</View></View>; }
