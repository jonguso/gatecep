 ...
 const [
  ...
  loadUnifiedPortfolioRuntime({
    ...
    return {
      ok: false,
      holdings: [],
      runtimeStatus: error?.code || "REAL_DATA_UNAVAILABLE",
...
setRealPortfolioNotice(
  unifiedResult?.runtimeStatus && unifiedResult.runtimeStatus !== "LIVE"
    ? {
        status: unifiedResult.runtimeStatus,
        message:
          unifiedResult.runtimeMessage ||
...
const portfolioSummary = useMemo(() => {
  const sharedResult =
    calculateSharedPortfolioSummary({
      holdings: activeHoldings,
      cash: Number(canonicalRealCash || 0)
    });
...
const blueprintCreated =
  ...
  const sourceLabel =
    realPortfolioNotice?.status === "AUTH_EXPIRED"
        ? "REAL PORTFOLIO • SESSION EXPIRED • LAST VERIFIED DATA"
      : realPortfolioNotice?.status === "AUTH_REQUIRED"
        ? "REAL PORTFOLIO • SIGN IN REQUIRED"
  ...
  async function selectPortfolioAccount(account) {
    try {
      ...
      setRealPortfolioNotice(
        nextPortfolio?.runtimeStatus && nextPortfolio.runtimeStatus !== "LIVE"
          ? {
              status: nextPortfolio.runtimeStatus,
              message:
                nextPortfolio.runtimeMessage ||
                "REAL portfolio data is temporarily unavailable."
            }
          : null
      );
    ...
    >
      ...
      {realPortfolioNotice ? (
        <View style={styles.realDataNotice}>
          <Text style={styles.realDataNoticeTitle}>
            REAL portfolio unavailable
          </Text>
          <Text style={styles.realDataNoticeText}>
            {realPortfolioNotice.message} Practice was not selected automatically.
          </Text>
          {realPortfolioNotice.status === "AUTH_EXPIRED" ||
          realPortfolioNotice.status === "AUTH_REQUIRED" ? (
            <Pressable
