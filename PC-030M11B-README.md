...
The backend uses a dedicated transactional replacement route rather than the additive portfolio POST route. Every verified adoption removes stale pre-identity, non-Practice rows, replaces the matching broker/client account, and preserves other identity-keyed broker accounts.
